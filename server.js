...
app.post('/api/download-receipt-pdf', async (req, res) => {
  ...
  try {
    ...
    const page = await browser2.newPage();
    await page.setViewport({ width: (width||300)+80, height: (height||900)+400, deviceScaleFactor: 2 });
...
<html><head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;font-weight:normal}
    ...
    const box = await el.boundingBox();
    const pdfWidth  = Math.ceil((box ? box.width  : width  || 300) + 40);
    const pdfHeight = Math.ceil((box ? box.height : height || 700) + 40);
